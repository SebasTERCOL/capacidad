...
CREATE OR REPLACE FUNCTION public.calculate_schedule_with_capacity(p_references text[], p_quantities numeric[], p_month integer, p_year integer, p_hours_per_shift numeric DEFAULT 7.83, p_operators jsonb DEFAULT '{}'::jsonb)
 RETURNS TABLE(referencia text, proceso_nombre text, proceso_id bigint, mejor_maquina text, cantidad numeric, sam numeric, duracion_min numeric, es numeric, ef numeric, ls numeric, lf numeric, slack numeric, is_critical boolean, makespan numeric, dias_habiles integer, operarios_proceso integer, minutos_disponibles numeric, porcentaje_capacidad numeric, desborda boolean, dias_overflow numeric)
 LANGUAGE plpgsql
...
BEGIN
  ...
  -- Forward Pass: set initial EF (WHERE true to satisfy pgBouncer)
  ...
  UPDATE _sched_nodes SET n_slack = l_start - e_start, is_crit = (l_start - e_start < 1.0) WHERE true;
