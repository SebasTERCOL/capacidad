...
RETURNS TABLE(
  referencia text, proceso_nombre text, proceso_id integer, mejor_maquina text,
  cantidad numeric, sam numeric, duracion_min numeric,
  es numeric, ef numeric, ls numeric, lf numeric, slack numeric, is_critical boolean,
  makespan numeric, dias_habiles integer,
  operarios_proceso integer, minutos_disponibles numeric, porcentaje_capacidad numeric,
...
BEGIN
  ...
  -- Track machine predecessor for backward pass
  DROP TABLE IF EXISTS _machine_pred;
  CREATE TEMP TABLE _machine_pred (
    node_id BIGINT,
    pred_node_id BIGINT
  );
  ...
  LOOP
    ...
    UPDATE _machine_timeline SET free_at = 0 WHERE true;
    TRUNCATE _machine_pred;

    -- Rebuild machine predecessor links each iteration
    DROP TABLE IF EXISTS _machine_order;
    CREATE TEMP TABLE _machine_order AS
    SELECT node_id, mach_name, e_start, e_finish, dur_min,
           row_number() OVER (PARTITION BY mach_name ORDER BY e_start, node_id) AS seq
    FROM _sched_nodes;

    INSERT INTO _machine_pred (node_id, pred_node_id)
    SELECT cur.node_id, prev.node_id
    FROM _machine_order cur
    JOIN _machine_order prev ON prev.mach_name = cur.mach_name AND prev.seq = cur.seq - 1;
  ...
  -- Build final machine predecessor links after RCPSP converged
  TRUNCATE _machine_pred;
  DROP TABLE IF EXISTS _machine_order;
  CREATE TEMP TABLE _machine_order AS
  SELECT node_id, mach_name,
         row_number() OVER (PARTITION BY mach_name ORDER BY e_start, node_id) AS seq
  FROM _sched_nodes;
  ...
  -- Machine-Aware Backward Pass
  -- Initialize LF = makespan for all nodes
  ...
  LOOP
    ...
    -- Propagate through process dependencies (successors)
    UPDATE _sched_nodes sn
    ...
    -- Propagate through machine successors
    UPDATE _sched_nodes sn
    SET l_finish = sub.succ_ls, l_start = sub.succ_ls - sn.dur_min
    FROM (
      SELECT mp.pred_node_id, succ.l_start AS succ_ls
      FROM _machine_pred mp
      JOIN _sched_nodes succ ON succ.node_id = mp.node_id
    ) sub WHERE sn.node_id = sub.pred_node_id AND sn.l_finish > sub.succ_ls;
    IF FOUND THEN v_changed := true; END IF;
  ...
  UPDATE _sched_nodes SET n_slack = l_start - e_start,
    is_crit = (ABS(l_start - e_start) < 1.0) WHERE true;
