...
BEGIN
  ...
  -- Serialize nodes that share the same machine.
  -- Use List Scheduling: process nodes in ES order;
  -- for each machine, track when it becomes free.
  -- ══════════════════════════════════════════════════

  -- Temp table to track machine availability
  ...
  -- Initialize all machines used
  INSERT INTO _machine_timeline (machine_name, free_at)
  SELECT DISTINCT mach_name, 0 FROM _sched_nodes;

  -- Iterate: assign each node respecting machine availability
  -- We may need multiple passes because pushing a node forward
  -- can affect its successors which may share machines with other nodes.
  v_iter := 0;
  LOOP
    v_iter := v_iter + 1;
    v_changed := false;

    -- Reset machine timelines
    UPDATE _machine_timeline SET free_at = 0;

    -- Process nodes in ES order, applying machine constraints
    FOR v_node IN
      SELECT node_id, ref, proc_id, mach_name, dur_min, e_start, e_finish
      FROM _sched_nodes ORDER BY e_start, proc_id, ref
    LOOP
      -- Get machine free time
      ...
      IF v_machine_free > v_node.e_start THEN
        -- Delay this node: its ES becomes when the machine is free
        UPDATE _sched_nodes
        ...
        UPDATE _machine_timeline
        SET free_at = v_machine_free + v_node.dur_min
        WHERE machine_name = v_node.mach_name;
      ELSE
        -- Machine is free, just update its timeline
        UPDATE _machine_timeline
        SET free_at = v_node.e_finish
        WHERE machine_name = v_node.mach_name;
      END IF;
    END LOOP;

    -- After machine pass, re-propagate precedence constraints
    -- (a delayed node may push its successors)
    LOOP
      UPDATE _sched_nodes sn
      SET e_start = sub.max_ef, e_finish = sub.max_ef + sn.dur_min
      FROM (
    ...
    -- Converge when no more changes or max iterations
    IF NOT v_changed OR v_iter >= v_max_iter THEN EXIT; END IF;
  END LOOP;

  -- ── Recalculate makespan after resource leveling ──
